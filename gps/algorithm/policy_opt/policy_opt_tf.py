...
class PolicyOptTf(PolicyOpt):
    ...
    def __init__(self, hyperparams, dO, dU):
        ...
        # self.device_string = "/cpu:0" # if not specified, tf will automatically define the best available option
    ...
    def update(self, obs, tgt_mu, tgt_prc, tgt_wt):
        ...
        # actual training.
        # decorator = tqdm.trange(self._hyperparams['iterations'], desc='NN (TF) training', leave=True)
        # for i in :
        # for i in decorator:
        for i in xrange(self._hyperparams['iterations']):
            ...
            average_loss += train_loss
            if (i+1) % 50 == 0:
                # decorator.set_description('tf average loss %f' % (average_loss/50))
                average_loss = 0
    ...
    def save_model(self, fname):
        ...
        self.saver.save(self.sess, save_path=fname, write_meta_graph=False)

    def restore_model(self, fname):
        print ('Restoring model from: %s', fname)
        LOGGER.debug('Restoring model from: %s', fname)
        self.saver.restore(self.sess, save_path=fname)

    # For pickling.
    def __getstate__(self):
        dir = self._hyperparams['weights_file_prefix']
        with tempfile.NamedTemporaryFile(dir=dir, mode='w+b', delete=False) as f:
    ...
    def _test(self):
        import numpy as np
        try:
            return np.load("/tmp/gps_test.npy")[0]
        except IOError:
            return None
    ...
    def __setstate__(self, state):
        ...
        #     dir = self._hyperparams['weights_file_prefix']
        # else:
        #     dir = '/media/cambel/Data/documents/gps/experiments/paper-08-18/' \
        #             + str(pol_dat).zfill(2) + '/policy'
        dir = self._hyperparams['weights_file_prefix']
        ...
        self.restore_model(tf.train.latest_checkpoint(dir))
