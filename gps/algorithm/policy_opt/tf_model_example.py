...
def init_weights(shape=None, name='Variable', initial_value=None):
    if initial_value is not None:
        return tf.Variable(initial_value=initial_value, name=name)
    else:
        return tf.Variable(initial_value=tf.random_normal(shape, stddev=0.01), name=name)


def init_bias(shape, name='Variable', initial_value=None):
    if initial_value is not None:
        return tf.Variable(initial_value=initial_value, name=name)
    else:
...
def get_mlp_layers(mlp_input, number_layers, dimension_hidden):
    ...
    for layer_step in range(0, number_layers):
        ...
        if layer_step != number_layers-1: # CB changed to ELU # final layer has no RELU
            cur_top = tf.nn.elu(tf.matmul(cur_top, cur_weight) + cur_bias)
        else:
...
def multi_modal_network_fp(dim_input=27, dim_output=7, batch_size=25, network_config=None):
    ...
    n_layers = 3
    layer_size = 40
    if 'units_layer' in network_config:
        print network_config
        layer_size = network_config['units_layer']
    dim_hidden = (n_layers - 1)*[layer_size]
    dim_hidden.append(dim_output)
    pool_size = 2
    filter_size = 3

    print "units per layer", layer_size
    ...
    im_width = network_config['image_width']
    num_channels = network_config['image_channels']
    image_input = tf.reshape(image_input, [-1, im_height, im_width, num_channels])
    # image_input = tf.reshape(image_input, [-1, num_channels, im_height, im_width])
    ...
    # CB - Load pretrained weights
    wc1,wc2,wc3,bc1,bc2,bc3 = None,None,None,None,None,None
    
    if 'load_weights' in network_config:
        Ws = np.load(network_config['wfile'])
        wc1 = tf.reshape(Ws[0], (filter_size, filter_size, num_channels,   num_filters[0])) #(3, 3, 3, 32))
        wc2 = tf.reshape(Ws[1], (filter_size, filter_size, num_filters[0], num_filters[1])) #(3, 3, 32, 32)
        wc3 = tf.reshape(Ws[2], (filter_size, filter_size, num_filters[1], num_filters[2])) #(3, 3, 32, 32)
        bc1 = Ws[3]
        bc2 = Ws[4]
        bc3 = Ws[5]
    ...
    with tf.variable_scope('conv_params'):
        weights = {
            'wc1': init_weights(shape=(filter_size, filter_size, num_channels,   num_filters[0]), initial_value=wc1), # 3x3 conv, 3 input, 32 outputs
            'wc2': init_weights(shape=(filter_size, filter_size, num_filters[0], num_filters[1]), initial_value=wc2), # 3x3 conv, 32 inputs, 32 outputs
            'wc3': init_weights(shape=(filter_size, filter_size, num_filters[1], num_filters[2]), initial_value=wc3), # 3x3 conv, 32 inputs, 32 outputs
        }

        biases = {
            'bc1': init_bias([num_filters[0]], initial_value=bc1),
            'bc2': init_bias([num_filters[1]], initial_value=bc2),
            'bc3': init_bias([num_filters[2]], initial_value=bc3),
        }
...
def conv2d(img, w, b, strides=[1, 1, 1, 1]):
    # relu -> elu. Allow negative numbers
    return tf.nn.elu(tf.nn.bias_add(tf.nn.conv2d(img, w, strides=strides, padding='SAME'), b))
